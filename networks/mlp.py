...
class SubnetMLPNet(nn.Module):
    def __init__(self, taskcla, sparsity, n_hidden=100, memory=0):
        ...
        self.quant_l1 = QuantLayer(weight_shape=self.fc1.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)

        self.fc2 = SubnetLinear(n_hidden, n_hidden, sparsity=sparsity, bias=False)
        self.quant_l2 = QuantLayer(weight_shape=self.fc2.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
