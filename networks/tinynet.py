...
class SubNet(torch.nn.Module):
    ...
    def make_layer(self, config, taskcla, sparsity):
        ...
        self.conv1 = subnet_conv3x3(in_planes, planes, stride, padding,
                                    sparsity=sparsity)
        self.quant1 = QuantLayer(weight_shape=self.conv1.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
        ...
        self.conv2 = subnet_conv3x3(in_planes, planes, stride, padding,
                                    sparsity=sparsity)
        self.quant2 = QuantLayer(weight_shape=self.conv2.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
        ...
        self.conv3 = subnet_conv3x3(in_planes, planes, stride, padding,
                                    sparsity=sparsity)
        self.quant3 = QuantLayer(weight_shape=self.conv3.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
        ...
        self.conv4 = subnet_conv3x3(in_planes, planes, stride, padding,
                                    sparsity=sparsity)
        self.quant4 = QuantLayer(weight_shape=self.conv4.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
        ...
        self.linear1 = SubnetLinear(in_planes, planes, bias=False,
                                    sparsity=sparsity)
        self.quant_l1 = QuantLayer(weight_shape=self.linear1.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
        ...
        self.linear2 = SubnetLinear(in_planes, planes, bias=False,
                                    sparsity=sparsity)
        self.quant_l2 = QuantLayer(weight_shape=self.linear2.weight.data.numpy().shape, quantization_procedure=linear_hist, word_length=16)
